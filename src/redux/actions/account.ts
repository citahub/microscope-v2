...
export type AccountAction = GET_ACCOUNT_TR_LIST | GET_ACCOUNT_ERC20_LIST | GET_ACCOUNT_BALANCE;
...
export function getERC20TransactionListByAccount(account:string, pageNum:number,pageSize:number) {
  return (dispatch:any) => {
    dispatch(showLoading())
    return dataAPI.getERC20TransactionListByAccount(account,pageNum,pageSize).then((data:any) => {
      dispatch(hideLoading())
      dispatch({
        type: constants.GET_ACCOUNT_ERC20_LIST,
        data: {
...
export function getBalance(account:string) {
  return (dispatch:any) => {
    dispatch(showLoading())
    return dataAPI.getBalance(account).then((data:any) => {
      dispatch(hideLoading())
      // alert(data);
      dispatch({
        type: constants.GET_ACCOUNT_BALANCE,
